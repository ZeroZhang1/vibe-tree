...
- 修复了同步后 XP 可能异常膨胀、来源显示不准确、模型占比不完整等问题。
...
- 修复多设备同步后 XP 可能被重复计算的问题。
- 修复同步数据来源显示成 cloud-sync 或 Cloud Tree 的问题。
- 修复 cache read 在部分历史同步数据里被重复计入的问题。
- 修复部分历史模型占比显示为 unknown 的问题，缺失模型时会显示为更清楚的未归类文案。
- 修复排行榜按钮和设置按钮语义容易混淆的问题。
...
- Fixed duplicate XP counting, source loss, cache-read inflation, and unclear model fallback labels.
